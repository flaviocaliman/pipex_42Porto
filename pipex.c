...
/*   Updated: 2023/09/21 17:23:50 by fgomes-c         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	child_process(char **av, char **envp, int *fd)
{
	...
	dup2(fd[1], STDOUT_FILENO); //redireciona a saida padrao STDOUT para fd[1]
	dup2(filein, STDIN_FILENO); //redireciona a entrada padrao STDIN para filein
	close(fd[0]);
	execute(av[2], envp);
}
...
int	main(int ac, char **av, char **envp)
{
	...
	if (ac == 5)
	{
		...
		waitpid(child_pid, NULL, 0); //wait the child_process
